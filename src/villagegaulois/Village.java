...
public class Village {
	...
	/*
	 * public String afficherVillageois() { StringBuilder chaine = new
	 * StringBuilder(); if (nbVillageois < 1) {
	 * chaine.append("Il n'y a encore aucun habitant au village du chef " +
	 * chef.getNom() + ".\n"); } else { chaine.append("Au village du chef " +
	 * chef.getNom() + " vivent les légendaires gaulois :\n"); for (int i = 0; i <
	 * nbVillageois; i++) { chaine.append("- " + villageois[i].getNom() + "\n"); } }
	...
	private static class Marche {
		...
		private Etal[] trouverEtals(String produit) {
			...
			for (int i = 0, index = 0; i < etals.length; i++) {
				if (etals[i].contientProduit(produit)) {
